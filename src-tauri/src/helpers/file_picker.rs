...
#[command]
pub async fn pick_bash_files(app: AppHandle) -> Result<Vec<BashFile>, String> {
...
pub async fn add_recent_file(app: AppHandle, file: BashFile) -> Result<(), String> {
    add_recent_files(app, vec![file]).await
}

#[command]
pub async fn add_recent_files(app: AppHandle, new_files: Vec<BashFile>) -> Result<(), String> {
    let store_path = app
        .path()
        .app_config_dir()
        .map_err(|e| format!("Failed to get config dir: {}", e))?
